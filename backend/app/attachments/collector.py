...
from app.attachments.files import download, image_url, save_file
...
class TourPhotos:
    ...
    def find(self, place):
        ...
        if source_id:
            rows = self.rows("detailCommon2", contentId=source_id, numOfRows=1)
            if any(str(row.get("contentid")) != source_id for row in rows):
                raise ProviderError("PHOTO_ID_MISMATCH")
            method = "provider_id"
        else:
            keyword = re.sub(r"(?:해변|계곡)$", "", place_name(place["name"]))
            if not keyword:
                return None, "no_match", "name_and_coordinates"
            rows = self.rows(
                "searchKeyword2", keyword=keyword, contentTypeId=12, numOfRows=20
            )
            rows = [
                r
                for r in rows
                if place_name(str(r.get("title", ""))) == place_name(place["name"])
                and distance(place, r) <= 500
            ]
            # Never resolve an ambiguous match by arbitrary search order.
            unique = {}
            for candidate in rows:
                code = str(candidate.get("contentid"))
                if code in unique and unique[code] != candidate:
                    raise ProviderError("CONFLICTING_PHOTO_SOURCE")
        ...
        if len(rows) != 1:
            return None, "ambiguous", method
        row = rows[0]
        source_id = str(row.get("contentid", ""))
        if not re.fullmatch(r"[0-9]{1,20}", source_id):
            raise ProviderError("INVALID_PHOTO_CONTENT_ID")
        url, license_code = row.get("firstimage"), row.get("cpyrhtDivCd")
        if not url:
            images = self.rows(
        ...
        if row.get("modifiedtime"):
            try:
                ...
                raise ProviderError("INVALID_PHOTO_SOURCE_TIME") from None
            if modified > datetime.now(UTC):
                raise ProviderError("FUTURE_PHOTO_SOURCE_TIME")
        title = str(row.get("title") or place["name"])[:200]
        return (
            dict(
...
def due_places(settings):
    with connect(settings) as c:
        ...
        return c.execute(
            ...
            SELECT w.*, p.source_id FROM places w
            LEFT JOIN LATERAL (
              SELECT source_id FROM pongdang_data.collection_place
              WHERE spot_id=w.id AND provider='TOURAPI_KOREAN'
              ORDER BY id LIMIT 1
            ) p ON true
...
def record_attempt(c, settings, spot_id, state, now, error=""):
    failed = state == "failed"
    row = c.execute(
    ...
    delay = (
        timedelta(minutes=min(1440, 30 * 2 ** min(failures - 1, 6)))
        if failed
        else timedelta(days=settings.photo_refresh_days)
    )
    c.execute(
        """
        INSERT INTO pongdang_data.attachment_collection
        (spot_id,state,checked_at,last_success_at,next_attempt_at,consecutive_failures,error_code)
        VALUES (%s,%s,%s,%s,%s,%s,%s) ON CONFLICT(spot_id) DO UPDATE SET
    ...
    """,
        [spot_id, state, now, None if failed else now, now + delay, failures, error],
    )
...
def persist(settings, place, photo, method, data, media_type, extension, now):
    ...
    with connect(settings) as c:
        ...
        record_attempt(c, settings, place["id"], "available", now)
...
def collect_photos(settings, *, provider=None, fetch_image=download):
    ...
    for place in due_places(settings):
        ...
        try:
            photo, state, method = provider.find(place)
            if photo:
                ...
                with connect(settings) as c:
                    ...
                    record_attempt(c, settings, place["id"], state, now)
        except (ProviderError, OSError) as exc:
            failed += 1
            code = exc.code if isinstance(exc, ProviderError) else "PHOTO_STORAGE_ERROR"
            with connect(settings) as c:
                record_attempt(c, settings, place["id"], "failed", now, code)
    return dict(
        received=received,
        inserted=inserted,
        state="failed"
