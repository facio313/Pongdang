...
const request = Object.freeze({
  dates: ['2026-09-21'], region: '강릉', preferred_tags: ['조용한 휴식'],
  activity: 'relax', transport: 'driving', keyword_selection: [],
});

test('foreign catalogue queries do not reuse the Korean default search text', () => {
  ...
  assert.equal(request.region, '강릉');
  assert.equal(requestInLanguage(request, 'ko').region, '강릉');
});
