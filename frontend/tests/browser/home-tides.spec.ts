...
for (const width of [390, 1440]) {
  test(`${width}px home shows tide order, KST dates and station scope alongside temperature criteria`, async ({ page }) => {
    ...
    await routeRecommendation(page, choice, { conditions: [{ ...base, condition_score: {
      ...
      components: [{ metric: "air_temperature", label: "외부 기온", value: 19.9, unit: "°C", score: 0,
    ...
    await expect(temperature).toContainText("자료 없음이나 활동 금지를 뜻하지 않습니다");
    await temperature.getByText("기온 점수 기준 · 미보정 참고값", { exact: true }).click();
    await expect(temperature).toContainText("21°C→0점");
    await expect(temperature.getByRole("link")).toHaveAttribute("href", "https://doi.org/10.3390/atmos7020030");
    await expect(temperature).toContainText("한국 시간별 방문 조건으로의 전이는 제품 가정");
