...
test("desktop home renders the same server-chosen activity and score as mobile", async ({ page }) => {
  ...
  // 히어로 점수는 여섯 활동 중 오늘 가장 좋은 하나입니다(useBestActivity).
  // 동점이면 이 순서가 우선순위입니다.
  ...
  await expect(page.locator(".hd-hero-title")).toContainText(bestLabel);
  ...
  await expect(page.locator(".hd-hero-lead .pd-score-reason")).toContainText("—");
  ...
  const swim = activities.find((item) => item.activity === "swim")!;
  const waterTemperature = swim.body.condition_score.components
