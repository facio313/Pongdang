...
const ROUTES = ["#home", "#today", "#spots", "#map", "#recommend"];
...
/** 지도(지점 · 코스 두 뷰)는 지도가 뷰포트를 다 쓰는 화면입니다.
 *
...
for (const [name, width] of [
  ...
  test(`${name} 지도는 페이지가 스크롤되지 않는다`, async ({ page }) => {
    await page.setViewportSize({ width, height: 900 });
    for (const route of ["#map", "#map?view=course"]) {
      await page.goto(route);
      await page.waitForLoadState("networkidle");
