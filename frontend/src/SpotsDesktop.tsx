    ...
    <DesktopShell>
      <DesktopHero
        nav={
          <DesktopNav
            active="spots"
            context={t("강릉 · 수집된 물놀이 장소 · {date}", { date: dateLabel() })}
          />
        }
        mascot="spot"
      >
      ...
      >
        <div className="sk-list">
          ...
          {rows.length > 0 && <div className="sk-list-foot"><span className="sk-note">{t("목록 {count}곳 전체입니다 · 서버가 한 번에 최대 100곳까지 내려줍니다", { count: rows.length })}</span></div>}
        </div>
      </LabelRow>
  ...
  return (
    <DesktopShell>
      ...
      <DesktopNav
        ...
        context={
          <>
            <a className="sk-crumb-link" href="#spots">{t("명소")}</a>{" "}
            <span className="sk-crumb-sep">›</span>{" "}
            {place ? t(kindLabel(place)) : t("분류 미확인")}{" "}
            <span className="sk-crumb-sep">›</span>{" "}
            {place?.name ?? (placeLoading ? t("조회 중") : t("장소 없음"))}
          </>
        }
      />
      ...
      <LabelRow
        ...
        desc={t("카카오 지도를 그대로 쓰고 핀 소스만 이 장소로 바꿉니다. 좌표는 서버가 준 실제 값이며, 없으면 찍지 않습니다.")}
      >
        <SplitBody columns="1.6fr 1fr">
          <div className="sk-detail-map">
            {markers.length ? (
              ...
              <div className="pd-dk-slot sk-detail-map-empty">{t("좌표가 아직 확인되지 않았습니다 · 지도 표시 없음")}</div>
            )}
          </div>
          <div>
              ...
              <a className="sk-nearby" href={spotLink(item)} key={item.id}>
                ...
                <span className="sk-nearby-body">
                  ...
                  <span className="sk-nearby-meta">
                    {t(kindLabel(item))} · {(item?.region ?? t("지역 미확인"))}
                  </span>
                </span>
              </a>
            ))}
