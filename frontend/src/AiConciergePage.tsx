...
function metadataValue(key: string, value: unknown): string {
  ...
  if (typeof value !== "string" && typeof value !== "number" && typeof value !== "boolean") return "상세 근거 참조";
  if (key === "activity" && typeof value === "string" && Object.hasOwn(activities, value)) return activities[value as keyof typeof activities];
  return dataStatuses[String(value)] ? `${dataStatuses[String(value)]} (${String(value)})` : String(value);
}
function Metadata({ value, depth = 0 }: { value: Record<string, unknown>; depth?: number }) {
  ...
  return <>
    {fields.length > 0 && <dl className="ai-metadata">{fields.map(([key, item]) => <div key={key}><dt>{metadataNames[key] ?? key}</dt><dd>{metadataValue(key, item)}</dd></div>)}</dl>}
    {sourceUrl && <a href={sourceUrl} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer">원본 출처 ↗</a>}
    {nested.map(([key, item]) => <details key={key}><summary>{key === "target" ? "대상 기간" : ["inputs", "evidence", "sources", "official_sources"].includes(key) ? "원본 근거의 시각·출처" : metadataNames[key] ?? key}</summary>{depth >= 3 ? <pre>{JSON.stringify(item, null, 2)}</pre> : (Array.isArray(item) ? item : [item]).slice(0, 100).map((child, index) => child && typeof child === "object" ? <Metadata key={index} value={child as Record<string, unknown>} depth={depth + 1} /> : null)}</details>)}
  </>;
}
function FactCard({ fact }: { fact: AiFact }) {
  return <article className="ai-fact">
    <div className="ai-badges"><span>{dataStatuses[fact.data_status] ?? fact.data_status}</span><span>{fact.feature}</span>{fact.metadata.station_id != null && <span>관측소 자료</span>}</div>
    <p className="ai-plain-text">{fact.text}</p>
    <Metadata value={fact.metadata} />
    <details><summary>근거 ID 확인</summary><p>근거 블록: {fact.fact_id}</p><ul>{fact.evidence_refs.map((ref) => <li key={ref}><code>{ref}</code></li>)}</ul></details>
  </article>;
}
export function AiResponseView({ response }: { response: ChatResponse }) {
  ...
  function candidateCards(ids: Set<string>) {
    return <div className="ai-candidates">{response.candidates.filter((candidate) => ids.has(candidate.candidate_id)).map((candidate) => <article className="ai-candidate" key={candidate.candidate_id}>
      ...
      <p className="table-note">장소 출처: {candidate.catalog_source ?? "기록 없음"} · 확인: {displayTime(candidate.catalog_verified_at)}</p>
      <nav aria-label={candidate.name + " 자료 화면"}>{candidate.links.map((link) => { const href = safeInternalLink(link.href); return href && <a key={href} href={href}>{link.label}</a>; })}</nav>
    </article>)}</div>;
  }
  return <div className="ai-response">
    ...
    {response.scope.queries.length > 0 && <details><summary>조회 범위</summary>{response.scope.queries.map((query, index) => <Metadata key={index} value={query} />)}</details>}
    ...
    {response.candidates.some((candidate) => !groupedCandidates.has(candidate.candidate_id)) && <section aria-label="실제 장소 후보"><h3>조건을 확인한 장소</h3>{candidateCards(new Set(response.candidates.filter((candidate) => !groupedCandidates.has(candidate.candidate_id)).map((candidate) => candidate.candidate_id)))}</section>}
    {response.facts.some((fact) => !groupedFacts.has(fact.fact_id)) && <section aria-label="추가 검증 근거"><h3>함께 확인할 근거와 제한</h3>{response.facts.filter((fact) => !groupedFacts.has(fact.fact_id)).map((fact) => <FactCard key={fact.fact_id} fact={fact} />)}</section>}
    {response.warnings.length > 0 && <section className="ai-notice"><h3>주의사항</h3><ul>{response.warnings.map((line, index) => <li key={index}>{line}</li>)}</ul></section>}
    {response.limitations.length > 0 && <section><h3>자료와 답변의 한계</h3><ul>{response.limitations.map((line, index) => <li key={index}>{line}</li>)}</ul></section>}
    {response.sources.length > 0 && <section><h3>출처</h3><ul>{response.sources.map((source) => { const url = safeSourceUrl(source.url); return <li key={source.id}>{url ? <a href={url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer">{source.name} ↗</a> : source.name} <code>{source.id}</code></li>; })}</ul></section>}
    <p className="table-note">조회 기능: {response.features.join(" · ") || "자료 조회 없음"} · 요청 ID: {response.request_id}</p>
  </div>;
}
...
export function AiConciergePage({ initialContext = {} }: { initialContext?: AiContext }) {
  ...
  return <article className="ai-concierge">
    <div className="ai-heading"><div><h1>AI에게 물어보기</h1><p>실제 Pongdang 자료로 장소·활동·시각을 바꿔가며 확인합니다.</p></div><button type="button" onClick={newConversation}>새 대화</button></div>
    <p className="ai-privacy">질문, 최근 대화와 필요한 공개 근거가 답변 처리를 위해 OpenAI에 전달됩니다. 개인정보를 입력하지 마세요. 대화는 이 화면의 메모리에만 유지되며, 새 대화나 페이지 이동 시 지워집니다. 제공자의 데이터 처리 정책은 별도로 적용됩니다.</p>
    <div className="ai-notice" role="status">{statusResult.error ?? (status ? aiStatusText(status) : "AI 설정 상태 확인 중…")}</div>
    {statusResult.error && <button type="button" onClick={() => { setStatusResult({}); setStatusRevision((value) => value + 1); }}>상태 다시 확인</button>}
    {status && !status.enabled && <p>아래 질문에는 가능한 기존 자료 조회 또는 조건 확인 안내로 응답합니다. AI가 생성한 답변으로 표시하지 않습니다.</p>}
    <p><a href="#data">데이터 조회</a> · <a href="#info">자료와 기능 정보</a></p>
    {turns.length === 0 && <section className="ai-examples" aria-label="예시 질문"><h2>이렇게 물어보세요</h2>{exampleQuestions.map((question) => <button key={question} type="button" onClick={() => { setDraft(question); input.current?.focus(); }}>{question}</button>)}</section>}
    <div className="ai-conversation" role="log" aria-label="대화 내용" aria-live="polite" aria-relevant="additions text">
      {turns.map((turn) => <section className="ai-turn" key={turn.id}><h2 className="ai-user-question">{turn.message}</h2>
        ...
        {turn.cancelled && <p className="table-note">요청을 취소했습니다. 이미 시작된 서버 처리와 비용은 취소되지 않을 수 있습니다.</p>}
        {!turn.response && !turn.error && !turn.cancelled && <p role="status">질문을 해석하고 실제 자료를 조회하고 있습니다…</p>}
      </section>)}
    </div>
    <form className="ai-form" onSubmit={(event) => { event.preventDefault(); void send(); }}>
      <details><summary>질문 맥락 {context.spot_id ? `· 장소 ID ${context.spot_id}` : ""}</summary>
        <div className="ai-context-fields"><label>지역<input value={context.region ?? ""} maxLength={60} onChange={(event) => setContext((value) => ({ ...value, region: event.target.value || undefined }))} disabled={busy} placeholder="예: 강릉" /></label>
          <label>활동<select value={context.activity ?? ""} onChange={(event) => setContext((value) => ({ ...value, activity: event.target.value as AiContext["activity"] || undefined }))} disabled={busy}><option value="">질문에서 해석</option>{Object.entries(activities).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></label>
          <label>대상 시각<input value={context.time_text ?? ""} maxLength={80} disabled={busy} placeholder="예: 내일 오후" onChange={(event) => setContext((value) => ({ ...value, time_text: event.target.value || undefined }))} /></label>
          {(context.spot_id || context.spot_ids?.length) && <button type="button" disabled={busy} onClick={() => setContext((value) => ({ ...value, spot_id: undefined, spot_ids: undefined }))}>장소 선택 해제</button>}
        </div><p className="table-note">맥락은 조회 조건이며 수치 근거가 아닙니다. 실제 장소와 시간 범위는 서버에서 다시 확인합니다.</p>
      </details>
      <label htmlFor="ai-question">질문</label>
      <textarea ref={input} id="ai-question" value={draft} maxLength={2000} rows={3} disabled={busy} aria-describedby="ai-input-help" onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && (event.ctrlKey || event.metaKey) && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} placeholder="지역, 활동, 궁금한 시간을 알려주세요." />
      <div className="ai-submit"><small id="ai-input-help">Enter 줄바꿈 · Ctrl/⌘ + Enter 전송 · {draft.length}/2,000자</small>{busy ? <button type="button" onClick={cancel}>요청 취소</button> : <button type="submit" disabled={!canSend || !draft.trim()}>질문 전송</button>}</div>
    </form>
  </article>;
}
