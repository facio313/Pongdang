  ...
  return (
    <article className="collector-doc">
      ...
      <p>
        ...
        pongdang_data에서 조회합니다. 과거 pongdang_demo 합성 예시는 은퇴했습니다.
        제공처별 설정·자료 존재·수집 상태는 실제 조회 결과를 확인하며, 데이터 조회가 수집을 실행하지 않습니다.
      </p>
      <section>
        ...
        <div className="table-scroll">
          <table>
            ...
            <tbody>
              <tr><td>제공처 어댑터</td><td>API 요청·응답을 공통 형식으로 변환</td><td>공식 제공처 어댑터 구현 · 실제 활성 상태는 수집 현황 참조</td><td>이 모듈에 주소·인증·필드 매핑 구현</td></tr>
              <tr><td>공통 수집·검증</td><td>제공처, 레코드 ID, 시각, 단위, 지표 검증</td><td>정규화된 근거와 원자적·멱등 저장</td><td>공통 입력 계약 유지</td></tr>
              ...
              <tr><td>예약 수집·평가</td><td>독립 worker와 저장된 공개 평가 조회</td><td>수집 상태·지원 범위는 데이터 조회에서 확인</td><td>수집 성공은 점수·안전 판정이 아님</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section>
        <h2>AI에게 물어보기</h2>
        <p><a href="#ai">자연어로 실제 자료 조회</a>에서 장소 검색, 수온·기상·예보·물때·수질·라이브캠 근거와 출처를 확인할 수 있습니다. 데이터 없는 추천·점수·안전 판정은 만들지 않으며, 알림 변경과 발송은 실행하지 않습니다.</p>
        <p>질문·최근 대화와 필요한 공개 근거는 OpenAI에 전달됩니다. AI 키는 서버에서만 설정하며 대화에 개인정보를 입력하지 마세요. AI가 준비되지 않았어도 기존 데이터 조회는 사용할 수 있습니다.</p>
      </section>
      <section>
