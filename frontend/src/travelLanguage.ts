...
/** The default Korean search text does not match foreign provider records.
 * Preserve explicit regions and all selected places; only remove that default
 * filter when querying a foreign catalogue.
 */
export function requestInLanguage(request: TravelRequest, locale: TravelLocale): TravelRequest {
  return {
    ...request,
    locale,
    region: locale !== "ko" && request.region === "강릉" ? undefined : request.region,
  };
}
