...
export function NotificationSettings({ onSaved }: { onSaved: () => void }) {
  ...
  const places = useResource<ClassifiedWaterPlace[]>("livecams/preview/places?q=" + encodeURIComponent(search));
  ...
  return (
    <>
      ...
      <p className="table-note">{t("해변·계곡으로 분류된 물놀이 장소만 표시합니다. 검색 결과는 최대 100곳이며, 장소 분류가 입수 안전을 뜻하지 않습니다.")}</p>
