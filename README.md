...
직접 통합하고 GitHub Actions에서 frontend와 backend 배포 이미지가
빌드되는지 확인한 뒤 자동 배포합니다. Actions는 변경 범위를 분류하거나
lint·단위·브라우저·DB·Compose 실행 검사를 하지 않습니다. frontend 이미지
빌드는 TypeScript 타입 검사와 Vite 생산 빌드를 포함하고, backend는 의존성
설치와 이미지 조립을 확인합니다. 이 두 이미지 빌드 성공만 CI 배포
게이트로 사용합니다. 배포 전 최신 main 여부를 확인하며 진행 중인 운영
배포는 새 빌드 때문에 취소하지 않습니다. 상세: [CI 운영](docs/ci.md).
...
관련 동작 검사는 변경 범위에 맞춰 로컬에서 명시적으로 실행합니다. Actions가 전체
검사를 대신하지 않으며, 반복 로컬 빌드는 기본 절차가 아닙니다.


실제 API를 호출하는 스모크는 수동 검증이며 Actions의 이미지 빌드는 제공처 응답을
검증하지 않습니다.
