...
**2026-09-14 후속 A1 구현:** 여섯 활동 선택·실제 조건 조회·사용자 명시 범위/중요도의 종합 조건 일치 점수를 추가하고 Water Index 허브에 연결했다. 현재 작업 트리 검증은 Backend 512개·Frontend 32개 통과다. 기존 과학적 환경 점수는 여전히 null이며 현장 외부 검증은 미완료다. 최신 A1 범위·API·화면·검증 결과는 [전체 활동 조건 점수 인계](activity_conditions.md)를 따른다. 아래는 그 이전 A1~A8 구현 기록이다.
