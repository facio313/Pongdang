...
## 그룹 B 백엔드 확장 (2026-09-15)

여행 추천·취향·기분·일정·동행의 최신 API와 계약은
[그룹 B 여행 백엔드](group-b-backend.md), 검증 결과는
[그룹 B 구현 및 검증 기록](group-b-implementation.md)을 따른다.
`/api/data/ai/chat`의 선택적 `travel` 상태로 폼/후속 대화를 연결하며,
`app.travel.chat`의 추가 읽기 도구가 서버의 추천 순서·일정·이벤트 결과 ID를 반환한다.
아래 기존 공개 자료 도구와 provider/SSO/비용 제한을 재사용한다.
이 확장 작업은 백엔드 범위이며 새 프런트 화면을 구현하지 않았다.
