...
- 조회 연결과 트랜잭션은 도구 내부에서 종료된다. OpenAI 대기 중 DB 연결을 보유하지 않는다.
  위 표의 공개 자료 조회 도구는 collector 실행·DB 수정·마이그레이션·외부 fetch·알림 발송을 수행하지 않는다.

## 여행 추천·경로 도구

`travel` 상태가 있는 대화는 [그룹 B 백엔드](group-b-backend.md)의 소유자별
취향·추천·일정·여행 세션 계약을 사용한다. 기존 SSO와 AI 비용 제한을 유지하며
모델의 도구는 개인 자료를 저장하거나 여행 세션을 시작하지 않는다.

- `travel_recommend`: 키워드·취향과 실제 관측/예보 근거로 장소·활동 목록을 반환한다.
  최초 목록 응답은 길찾기 API를 호출하지 않는다.
- `travel_route`: 별도 경로 요청과 이전 선택 토큰이 있을 때 카카오모빌리티를
  제한적으로 호출한다. 예상 방문 시각의 환경 자료와 후보 방문 순서를 비교한다.
  선택된 후보 범위·누락 근거를 표시하며 안전 또는 지역 전체 최적을 보장하지 않는다.
- 전체 프런트 요청·응답 필드, 키 설정과 호출 상한은
