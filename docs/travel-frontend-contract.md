...
편집한 TravelRequest 전체를 보낸다. 출발지는 사용자가 고른 좌표 또는 실제 등록
spot_id가 필요하다. 주소 문자열만으로 임의 좌표를 만들지 않는다.
...
- `route.legs[].geometry.polyline`: 선택 후 조회한 경도/위도 선. 순서대로 연결
...
검사한 마지막 요청은 2026-09-15 17:00 KST에 HTTP 401이었다.
401/403에서 후속 호출을 멈추고 인증 실패를 보존하는 동작을 확인했다.
이 결과로 운영 키의 상태를 추정하지 않으며 실제 운영 성공으로 보고하지 않는다.
키 자체와 외부 응답 원문은 기록하지 않았다.
