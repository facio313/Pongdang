...
전체 브라우저 검사는 두 runner의 독립 DB로 나누고, 각 shard의 worker는 1개를 유지한다.

백엔드 전체 검사는 세 runner의 독립 PostgreSQL `pongdang_test`로 나눈다.
각 runner 안에서는 순차 실행하며 같은 DB에 병렬 worker를 붙이지 않는다.
빠른 관련 테스트 경로는 기존대로 한 runner만 사용한다. 모든 shard가 통과해야
배포할 수 있고 한 shard의 실패가 다른 shard의 검사를 취소하지 않는다.

`ops/backend_shard.py`는 pytest의 실제 수집 결과를 파일 단위로 나눈다.
`ops/backend_test_weights.json`의 과거 파일별 소요 시간 추정치를 기준으로 오래 걸리는
파일부터 배분하며, 기록이 없는 새 파일도 테스트 수 기반 추정치로 자동 포함한다.
초기 추정치는 main CI 35590477900의 콘솔 완료 시각 간격으로 만들었으므로 정확한
테스트별 실행 시간은 아니다. 추정치는 작업 배분에만 쓰고 검사 생략에 쓰지 않는다.
각 shard는 선택/전체 테스트 수와 전체 수집 목록 해시, 느린 테스트 15개를 로그에 남긴다.
분할 변경 시 실제 수집 목록의 누락·중복 없음과 전체 통과 수를 비교한다.
