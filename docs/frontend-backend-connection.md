...
- 2026-09-16 당시에는 CI의 disposable DB에서 브라우저 회귀검사도 실행하도록
  연결했다. 이는 기존 검증 구성의 기록이며, 2026-09-23 이후 Actions는 배포
  이미지 빌드만 수행하고 브라우저 회귀검사를 실행하지 않는다. 필요한 회귀검사는
  아래의 폐기 가능한 로컬 DB로 명시적으로 실행하며, 로컬 통과와 원격 빌드/배포는 별개다.
